...
- **Bing index presence** — checks whether the page appears to be indexed by Bing, since Microsoft Copilot answers from Bing's index (ChatGPT runs its own index, so this is not a ChatGPT signal). Verdicts are deliberately conservative: an inconclusive result is reported as inconclusive rather than guessed, and a "not listed" verdict is only shown when the check has real signal to support it.
