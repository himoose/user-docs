...
Tout cela se trouve dans **Paramètres → Site Monitoring**, délimité au projet en cours.

## L'exploration planifiée

Activez **Run weekly site crawl** et Hi, Moose actualisera les pages surveillées à une cadence hebdomadaire, en vous avertissant lorsque des changements significatifs apparaissent. Choisissez le jour et l'heure dans votre horaire local.

Les explorations manuelles restent disponibles dans tous les cas. **Run crawl now** lance immédiatement une exploration complète, avec la progression dans la barre d'état en bas de la fenêtre. Vous pouvez **arrêter** une exploration en cours : les pages trouvées jusque-là sont conservées plutôt que jetées.

Les grands sites prennent quelques minutes.

## L'index de recherche

Sous les commandes d'exploration se trouve le **Search index**, l'index sémantique local que Moose utilise pour répondre aux questions sur les pages de votre site. Il indique combien de pages sont indexées, combien de passages ces pages ont produits et à quand remonte sa dernière mise à jour.
...
- **Rebuild index** réindexe entièrement.
...
Les niveaux d'impact sont **Critical**, **Notable**, **Low**, **Unreachable** et **Not reviewed**.

Deux réglages régulent le bruit :

- **Include informational AEO changes** : désactivé par défaut. Une fois activé, les changements à faible impact génèrent également des éléments dans l'Inbox.
- **OS notification minimum severity** : seuls les éléments de l'Inbox situés à ce niveau ou au-dessus déclenchent une notification du bureau. Réglez-le sur Info si vous souhaitez que les changements à faible impact remontent aussi au niveau du système d'exploitation.

## Demander à Moose ce qui a changé

Le récapitulatif comporte une action **Ask Moose about all of this** qui transmet l'instantané complet de l'exploration au chat et lui demande ce qui compte le plus et par quoi commencer. Les pages individuelles disposent de leur propre action **Ask Moose**, qui ne transmet que le changement enregistré pour cette page.
...
Si une page ou l'ensemble du site est inaccessible, cela est signalé comme un type de changement à part entière au lieu d'être ignoré en silence. Une panne générale du site produit un unique élément **Site unreachable** au lieu d'une alerte par page.
